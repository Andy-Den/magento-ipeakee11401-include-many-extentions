    ...
    function getCreateMarkerJS() {
    	$_output = "
    	   ...
    	   function createMarker(map, point, title, html, icon, icon_shadow, sidebar_id, openers){
			    ...
			    if(html!=''){
					".(($this->info_window)?"
			        var infowindow = new google.maps.InfoWindow({content: html,zindex:1000});
					...
					if(openers != ''&&!isEmpty(openers)){
			            for(var i = 0; i < openers.length; i++){
			             ...
			             opener.on".$this->window_trigger." = function(){	
						 		if (!isEmpty(lastInfoWindow)) {lastInfoWindow.close();}
						        lastInfoWindow = infowindow;
								infowindow.open(map,new_marker);								
								new Effect.ScrollTo('showmap');
							 	return false
							 };
			           }
			        }
