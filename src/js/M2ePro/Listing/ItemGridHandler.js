...
ListingItemGridHandler.prototype = Object.extend(new CommonHandler(), {
    ...
    viewItemHelp : function(rowId, data)
    {
        ...
        var self = this;
        $$('.hide_item_help').each(function(s) {
            s.observe('click', function() {
    ...
    createHelpTitleHtml : function(rowId)
    {
        ...
        var closeHtml = '<a href="javascript:void(0);" class="hide_item_help" title="'+self.M2ePro.text.close_word+'"><span class="hl_close">&times;</span></a>';
    ...
    {
        var self = this;
        var productId = strip_tags(this.getProductIdByRowId(rowId));

        var url = '';
        if (this.gridId == 'ebayListingOtherGrid') {
            url = self.M2ePro.url.logViewUrl+'filter/'+base64_encode('item_id='+productId);
        } else {
