...
AmazonAccountHandler.prototype = Object.extend(new CommonHandler(), {
    ...
    initialize: function()
    {
        ...
        Validation.add('M2ePro-require-select-marketplaces', M2ePro.text.need_select_marketplaces_error, function(value, el) {
            ...
            $$('.marketplace_mode_select').each(function(obj) {
                if (obj.value == 1) {
    ...
    changeMarketplaceMode : function(id)
    {
        var self = AmazonAccountHandlerObj;

        if ($('marketplace_mode_'+id).value == 0) {
            $('marketplaces_register_url_container_'+id).hide();
            $('marketplaces_application_name_container_'+id).hide();
            $('marketplaces_developer_key_container_'+id).hide();
            $('marketplaces_merchant_id_container_'+id).hide();
            $('marketplaces_marketplace_id_container_'+id).hide();
            $('marketplaces_related_store_id_container_'+id).hide();
            $('marketplace_mode_'+id).removeClassName('hightlight_selected');
            $('marketplace_mode_'+id).addClassName('lacklustre_selected');
        } else {
            ...
            $('marketplaces_marketplace_id_container_'+id).show();
            $('marketplaces_related_store_id_container_'+id).show();
            $('marketplace_mode_'+id).removeClassName('lacklustre_selected');
            $('marketplace_mode_'+id).addClassName('hightlight_selected');
        }
    },
    ...
    mapping_general_id_mode_change : function()
    {
        ...
        self.change_visibility_for_other_listings_move_mode();
    },
    ...
    change_visibility_for_other_listings_move_mode : function()
    {
        var self = AmazonAccountHandlerObj;
        var isVisible = $('mapping_general_id_mode').value != self.OTHER_LISTINGS_MAPPING_GENERAL_ID_MODE_NONE ||
                        $('mapping_sku_mode').value != self.OTHER_LISTINGS_MAPPING_SKU_MODE_NONE ||
                        $('mapping_title_mode').value != self.OTHER_LISTINGS_MAPPING_TITLE_MODE_NONE;
    ...
    magentoOrdersCustomerModeChange : function()
    {
        ...
        $('magento_orders_customer_new_newsletter_mode_container')[action]();
