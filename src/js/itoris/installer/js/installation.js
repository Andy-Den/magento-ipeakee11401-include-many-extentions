	...
	ITask.prototype.checkDirectoryPermissions = function(response){
		...
		if(response == undefined){
			...
			for(var i = 0; i < dirs.length; i++){
				...
				if(mustAdd == true){
					var dirName = dir.attributes.getNamedItem('name').childNodes.item(0).nodeValue;
					var dirPath = '';
					var node = dir;
					while(node.attributes.getNamedItem('name').childNodes.item(0).nodeValue != 'root'){
						dirPath = '/' + node.attributes.getNamedItem('name').childNodes.item(0).nodeValue + dirPath;
	...
	ITask.prototype.getFoldersStack = function(node){
		var result = new Array();
		while(node.attributes.getNamedItem('name').childNodes.item(0).nodeValue != 'root'){
			result.unshift(node.attributes.getNamedItem('name').childNodes.item(0).nodeValue);
			node = node.parentNode;
		}
		return result;
	};
	...
	ITask.prototype.findNode = function(root, names){
		...
		while(names.length > 0){
			...
			for(var i = 0; i < node.childNodes.length; i++){
				...
				if(childNode.attributes.getNamedItem('name').childNodes.item(0).nodeValue == lookFor){
					node = childNode;
					founded = true;
					break;
				}
		...
		if(response == undefined){
			...
			for(var i = 0; i < fileNodes.length; i++){
				...
				if(localFileNode == null){
					...
					var md5local = localFileNode.attributes.getNamedItem('md5').childNodes.item(0).nodeValue;
					var md5remote = fileNode.attributes.getNamedItem('md5').childNodes.item(0).nodeValue;
		...
		if(response == undefined){
			...
			while( i<f_cnt ){
				...
				while( i < f_cnt )
				{
					...
					var file_size = file.attributes.getNamedItem( 'size' ).childNodes.item(0).nodeValue;
	...
	DownloadJob.prototype.execute = function(response){
		if(response == undefined){
			...
			for( var i=0; i<p_cnt; i++ )
			{
				...
				md5s.push( this.task.joblist[this.n].fileNode[ i ].attributes.getNamedItem('md5').childNodes.item(0).nodeValue );
			}
			installer.sendApiRequest({
