...
Accordion.prototype = {
    ...
    sectionClicked: function(event) {
        ...
        var currentElement = $(Event.element(event)).up('.section');
        $$('.opc li .step-title h2').each(function(h2){
            if(h2.hasClassName('step-next')){
                h2.removeClassName('step-next');
            }
        });
        _nextElment = currentElement.next('li');
        _nextElment.down('div.step-title h2').addClassName('step-next');
        Event.stop(event);
    },
