...
Checkout.prototype = {
    initialize: function(accordion, urls){
        ...
        //Cufon.replace('.opc .step-title h2',{fontFamily: 'AvenirBold'});
    },
    ...
    /*setMethod: function(){
        alert('*******');
        if ($('guest-email').value && $('guest-name').value && $('guest-telephone').value) {
            this.method = 'guest';
            var request = new Ajax.Request(
                this.saveMethodUrl,
                {method: 'post', onFailure: this.ajaxFailure.bind(this), parameters: {'method':'guest'}}
            );
            $('billing:email').value = $('guest-email').value;
            $('billing:firstname').value = $('guest-name').value;
            $('billing:telephone').value = $('guest-telephone').value;
            Element.hide('register-customer-password');
            this.gotoSection('billing');            
        
        }
        else{
            alert(Translator.translate('Please fill in the details above to checkout as a guest'));
            return false;
        }
    },*/
