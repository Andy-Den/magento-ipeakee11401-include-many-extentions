...
Enterprise.Wishlist.Widget.Form = Class.create(Enterprise.Widget, {
    ...
    initialize: function($super, action) {
        var _templateString = '<ul class="form-list">' +
                ...
                '<li class="control"><div class="input-box wishlistformark-check"><input type="checkbox" id="wishlist-public" name="visibility"></div><label class="wishlistformark" for="wishlist-public">' + Translator.translate('Make This Wishlist Public') + '</label></li>' +
            '</ul>' +
            '<div class="buttons-set form-buttons"><button type="submit" class="button btn-save allajaxcreatewish"><span><span>' + Translator.translate('Save') + '</span></span></button><button type="button" class="button btn-cancel"><span><span>' + Translator.translate('Cancel') + '</span></span></button></div>';
...
Enterprise.Wishlist.createWithCallback = function(createUrl, callback, useAjax) {
    ...
    if (!Enterprise.Wishlist.createWithCallbackDialog) {
        ...
        Enterprise.Wishlist.createWithCallbackDialog.form.onSubmit = Enterprise.Wishlist.createWithCallbackDialog.form.onSubmit.wrap(function(proceed, event) {
            ...
            if (this.isValid) {
                //alert('dsds');
                Enterprise.Wishlist.createWithCallbackDialog.hide();
                jQuery('.popup-block .block-content form')[0].reset();
            }
        })
    }
...
Event.observe(document, 'dom:loaded', function() {
    if (typeof Enterprise.Wishlist.list != 'undefined'
        ...
        $$('.link-wishlist').each(function(link) {
            ...
            var url = null;
            var id = null;
            if (link.href && link.href != '#' &&  link.href.length > 0 ){
                url = link.href;
            }
            else {
                var  mystring = jQuery(link).attr('onclick');
                if (typeof mystring != 'undefined' ){
                    jQuery(link).removeAttr('onclick');
                    mystring = mystring.toString();
                    mystring = mystring.match(/\((.*)\)/);
                    mystring = mystring[1].replace(/'/g,"");
            ...
            var onclick = link.onclick || function() {
                if (id) { 
                    if(jQuery('body').hasClass('catalog-product-view')){
                        ajaxcart.showProductViewWishlistPopup(this.href,id);
                    }else{
                        setLocation(this.href,id);
                    }
                }
                else{
