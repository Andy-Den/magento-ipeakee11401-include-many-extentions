...
function colorSelected(id, value, product_image_src, front_label) {
		...
		if (theswitchis === 'on') {
			...
			if (isAswatch == 1){
				...
				$(thedropdown).select('option').each(function(option) {
                    ...
                    $$('#ul-moreviews li.moreview' + option.value).invoke('removeClassName', 'currentSws');
				});
	...
	if (value) {
		...
		if (selectedmoreview !== null && selectedmoreview !== undefined && howMany > 0) {
			if(selectedmoreviewtitle !== null && selectedmoreviewtitle !== undefined) {
				selectedmoreviewtitle.show();
			}
			selectedmoreview.invoke('show');
            selectedmoreview.invoke('addClassName', 'currentSws');
            $$('#ul-moreviews li.onload-moreviews').invoke('hide');
    ...
    // Hide validation require

    $("advice-required-entry-" + id).fade();

    // Reload Slider
    jQuery("#ul-moreviews").prepend(jQuery(".currentSws"));
    currentSelector = 'li.currentSws';
    reloadThumbSlider();
