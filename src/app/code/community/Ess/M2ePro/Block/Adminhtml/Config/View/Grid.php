...
class Ess_M2ePro_Block_Adminhtml_Config_View_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    {
        $data = $row->getData();
        $value = '<a href="javascript:void(0);" onclick="ConfigHandlerObj.setForUpdate(\''.$data['id'].'\',\''.Mage::helper('M2ePro')->escapeJs($data['group']).'\',\''.Mage::helper('M2ePro')->escapeJs($data['key']).'\',\''.Mage::helper('M2ePro')->escapeJs($data['value']).'\',\''.Mage::helper('M2ePro')->escapeJs($data['notice']).'\');">'.Mage::helper('M2ePro')->__('Edit').'</a>';
        $value .= '&nbsp;&nbsp;&nbsp;&nbsp;';
        $value .= '<a href="javascript:void(0);" onclick="ConfigHandlerObj.removeConfig(\''.Mage::helper('M2ePro')->escapeJs($data['group']).'\',\''.Mage::helper('M2ePro')->escapeJs($data['key']).'\');">'.Mage::helper('M2ePro')->__('Delete').'</a>';
        return $value;
    }

    public function getGridUrl()
    {
        return $this->getUrl('*/*/grid', array('_current'=>true,'mode'=>Mage::helper('M2ePro')->getGlobalValue('config_mode')));
    }
