...
class Ess_M2ePro_Block_Adminhtml_Amazon_Template_Description_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        if (Mage::helper('M2ePro')->getGlobalValue('temp_data') && Mage::helper('M2ePro')->getGlobalValue('temp_data')->getId()) {
            $this->_headerText = Mage::helper('M2ePro')->__("Edit{$componentName} Description Template");
            $this->_headerText .= ' "'.$this->escapeHtml(Mage::helper('M2ePro')->getGlobalValue('temp_data')->getTitle()).'"';
        } else {
            $this->_headerText = Mage::helper('M2ePro')->__("Add{$componentName} Description Template");
        }
        //------------------------------
        ...
        $this->_addButton('back', array(
            ...
            'onclick'   => 'AmazonTemplateDescriptionHandlerObj.back_click(\'' .Mage::helper('M2ePro')->getBackUrl('list').'\')',
            'class'     => 'back'
        ));
