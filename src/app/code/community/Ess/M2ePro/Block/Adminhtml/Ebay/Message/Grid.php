...
class Ess_M2ePro_Block_Adminhtml_Ebay_Message_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    public function callbackColumnMessage($value, $row, $column, $isExport)
    {
        ...
        if (strlen($messageText) > 135) {
            ...
            $messageText .= ' <a href="javascript:void(0);" onclick="EbayMessageHandlerObj.showMessageText(\''.$row->getId().'\')">'.Mage::helper('M2ePro')->__('Read More').'</a>';
        }
    ...
    public function callbackColumnResponses($value, $row, $column, $isExport)
    {
        ...
        foreach ($responses as $key => $response) {
            ...
            if (strlen($response) > 60) {
                ...
                $responseHtml .= ' <a href="javascript:void(0);" onclick="EbayMessageHandlerObj.showMessageResponse(\''.$row->getId().'\',
        ...
        $buttonBlock = $this->getLayout()
                            ...
                            ->setData( array(
                                ...
                                'onclick' => 'EbayMessageHandlerObj.openMessage(this,
                                                                              \''.$row->getData('id').'\',
