...
class Ess_M2ePro_Block_Adminhtml_Ebay_Listing_Filter extends Mage_Adminhtml_Block_Widget
{
    ...
    protected function _beforeToHtml()
    {
        ...
        //-------------------------------
        $this->selectedSellingFormatTemplate = (int)$this->getRequest()->getParam('filter_ebay_selling_format_template');
        $sellingFormatTemplatesCollection = Mage::helper('M2ePro/Component_Ebay')->getCollection('Template_SellingFormat')
                                                                            ->setOrder('title', 'ASC');
        ...
        if ($sellingFormatTemplatesCollection->getSize() < $maxRecordsQuantity) {
            ...
            if ($this->selectedSellingFormatTemplate > 0) {
                $this->selectedSellingFormatTemplateValue = Mage::helper('M2ePro/Component_Ebay')->getModel('Template_SellingFormat')->load($this->selectedSellingFormatTemplate)->getTitle();
            } else {
        ...
        if ($descriptionsTemplatesCollection->getSize() < $maxRecordsQuantity) {
            ...
            if ($this->selectedDescriptionTemplate > 0) {
                $this->selectedDescriptionTemplateValue = Mage::helper('M2ePro/Component_Ebay')->getModel('Template_Description')->load($this->selectedDescriptionTemplate)->getTitle();
            } else {
        ...
        if ($generalTemplatesCollection->getSize() < $maxRecordsQuantity) {
            ...
            if ($this->selectedGeneralTemplate > 0) {
                $this->selectedGeneralTemplateValue = Mage::helper('M2ePro/Component_Ebay')->getModel('Template_General')->load($this->selectedGeneralTemplate)->getTitle();
            } else {
        ...
        //-------------------------------
        $this->selectedSynchronizationTemplate = (int)$this->getRequest()->getParam('filter_ebay_synchronization_template');
        $synchronizationsTemplatesCollection = Mage::helper('M2ePro/Component_Ebay')->getCollection('Template_Synchronization')
                                                                               ->setOrder('title', 'ASC');
        ...
        if ($synchronizationsTemplatesCollection->getSize() < $maxRecordsQuantity) {
            ...
            if ($this->selectedSynchronizationTemplate > 0) {
                $this->selectedSynchronizationTemplateValue = Mage::helper('M2ePro/Component_Ebay')->getModel('Template_Synchronization')->load($this->selectedSynchronizationTemplate)->getTitle();
            } else {
