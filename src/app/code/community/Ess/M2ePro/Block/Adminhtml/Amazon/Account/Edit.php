...
class Ess_M2ePro_Block_Adminhtml_Amazon_Account_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        if (Mage::helper('M2ePro')->getGlobalValue('temp_data') && Mage::helper('M2ePro')->getGlobalValue('temp_data')->getId()) {
            $this->_headerText = Mage::helper('M2ePro')->__("Edit{$componentName} Account");
            $this->_headerText .= ' "'.$this->escapeHtml(Mage::helper('M2ePro')->getGlobalValue('temp_data')->getTitle()).'"';
        } else {
            $this->_headerText = Mage::helper('M2ePro')->__("Add{$componentName} Account");
        }
        //------------------------------
        ...
        if ($isWizardActive && Mage::getSingleton('M2ePro/Wizard')->getStatus($mode) == Ess_M2ePro_Model_Wizard::STATUS_ACCOUNTS_AMAZON) {
