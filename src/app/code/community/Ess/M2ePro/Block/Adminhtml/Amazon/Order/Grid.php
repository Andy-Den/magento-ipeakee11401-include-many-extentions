...
class Ess_M2ePro_Block_Adminhtml_Amazon_Order_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('status', array(
            ...
            'type'    => 'options',
            'options' => array(
                Ess_M2ePro_Model_Amazon_Order::STATUS_PENDING             => Mage::helper('M2ePro')->__('Pending'),
                Ess_M2ePro_Model_Amazon_Order::STATUS_UNSHIPPED           => Mage::helper('M2ePro')->__('Unshipped'),
                Ess_M2ePro_Model_Amazon_Order::STATUS_SHIPPED_PARTIALLY   => Mage::helper('M2ePro')->__('Partially Shipped'),
                Ess_M2ePro_Model_Amazon_Order::STATUS_SHIPPED             => Mage::helper('M2ePro')->__('Shipped'),
                Ess_M2ePro_Model_Amazon_Order::STATUS_INVOICE_UNCONFIRMED => Mage::helper('M2ePro')->__('Invoice Not Confirmed'),
                Ess_M2ePro_Model_Amazon_Order::STATUS_UNFULFILLABLE       => Mage::helper('M2ePro')->__('Unfulfillable'),
                Ess_M2ePro_Model_Amazon_Order::STATUS_CANCELED            => Mage::helper('M2ePro')->__('Canceled')
            ),
    ...
    public function callbackColumnMagentoOrder($value, $row, $column, $isExport)
    {
        ...
        if ($magentoOrderId !== '' && $magentoOrderId !== null) {
            ...
            if ($row['magento_order_num']) {
                $orderUrl = Mage::helper('adminhtml')->getUrl('adminhtml/sales_order/view', array('order_id' => $magentoOrderId), null);
                $returnString = '<a href="' . $orderUrl . '" target="_blank">' . $row['magento_order_num'] . '</a>';
            } else {
                $returnString = '<span style="color: red;" title="'.Mage::helper('M2ePro')->__('Deleted').'">'.$magentoOrderId.'</span>';
            }
    ...
    private function getViewLogIconHtml($orderId)
    {
        ...
        $html .= '<a title="'.$iconTip.'" id="orders_grid_help_icon_open_'.(int)$orderId.'" href="javascript:void(0);" onclick="OrderHandlerObj.viewOrderHelp('.(int)$orderId.',\''.base64_encode(json_encode($logRows)).'\', \''.$this->getId().'\');"><img src="'.$iconSrc.'" /></a>';
        $html .= '<a title="'.$iconTip.'" id="orders_grid_help_icon_close_'.(int)$orderId.'" style="display:none;" href="javascript:void(0);" onclick="OrderHandlerObj.hideOrderHelp('.(int)$orderId.', \''.$this->getId().'\');"><img src="'.$iconSrc.'" /></a>';
        $html .= '</span>';
...
HTML;
    ...
    {
        return Mage::helper('M2ePro')->convertCurrencyNameToCode($row->getData('currency'), $row->getData('paid_amount'));
    }
    ...
    public function callbackColumnStatus($value, $row, $column, $isExport)
    {
        ...
        $statusColors = array(
            ...
            Ess_M2ePro_Model_Amazon_Order::STATUS_UNSHIPPED           => '#9acd32',
            Ess_M2ePro_Model_Amazon_Order::STATUS_SHIPPED_PARTIALLY   => 'lightgreen',
            Ess_M2ePro_Model_Amazon_Order::STATUS_SHIPPED             => 'green',
            Ess_M2ePro_Model_Amazon_Order::STATUS_UNFULFILLABLE       => 'purple',
            Ess_M2ePro_Model_Amazon_Order::STATUS_INVOICE_UNCONFIRMED => 'purple',
        ...
        if ($row->isLockedObject('update_order_status')) {
            $value .= '<br />';
            $value .= '<span style="color: gray;">['.Mage::helper('M2ePro')->__('Status Update in Progress...').']</span>';
        }
    ...
    {
        return $this->getUrl('*/adminhtml_amazon_order/view', array('id' => $row->getId(),'back'=>Mage::helper('M2ePro')->makeBackUrlParam('*/adminhtml_order/index', array('tab' => Ess_M2ePro_Block_Adminhtml_Component_Abstract::TAB_ID_AMAZON))));
    }
