...
class Ess_M2ePro_Adminhtml_M2epro_GeneralController extends Ess_M2ePro_Controller_Adminhtml_MainController
{
    //#############################################
    ...
    {
        exit( Mage::getModel('M2ePro/Connector_Server_Api_Dispatcher')
                        ->processVirtual('service','get','notice',
                                         array('key'=>$this->getRequest()->getParam('key')),'value') );
    }
    ...
    public function searchAutocompleteAction()
    {
        ...
        $relevanceQueryString = "IF( `main_table`.`title` LIKE '%". $quotedQueryString. "%', ".(substr_count($quotedQueryString, " ") + 1). "*3, 0) + IF( `main_table`.`title` LIKE '%".str_replace(" ", "%', 1, 0) + IF( `main_table`.`title` LIKE '%", $quotedQueryString). "%', 1 , 0)";

        $collection = Mage::helper('M2ePro/Component')
            ->getComponentModel($component, $model)
            ->getCollection()
            ->addExpressionFieldToSelect('relevance', $relevanceQueryString, null)
        ...
        foreach ($results as $result) {
            $suggestions[] = Mage::helper('M2ePro')->escapeHtml($result['title']);
            $ids[] = $result['id'];
        }
        $array = array(
            'query'       => $queryString,
    ...
    public function searchAutocompleteByAttributeSetIdAction()
    {
        ...
        $queryString = $this->getRequest()->getParam('query');
        $maxResults  = (int) $this->getRequest()->getParam('maxResults');
        $attributeSets = (int) $this->getRequest()->getParam('attribute_sets');
