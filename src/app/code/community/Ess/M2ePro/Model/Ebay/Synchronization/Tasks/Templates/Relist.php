...
class Ess_M2ePro_Model_Ebay_Synchronization_Tasks_Templates_Relist extends Ess_M2ePro_Model_Ebay_Synchronization_Tasks
{
    ...
    private function immediatelyChangeEbayStatus()
    {
        ...
        foreach ($changedListingsProducts as $changedListingProduct) {
            ...
            if ($listingProduct->isRelistable()) {
                ...
                $this->_runnerActions->setProduct($listingProduct,Ess_M2ePro_Model_Connector_Server_Ebay_Item_Dispatcher::ACTION_RELIST,$tempParams);
            } else if ($listingProduct->isListable()) {
                $this->_runnerActions->setProduct($listingProduct,Ess_M2ePro_Model_Connector_Server_Ebay_Item_Dispatcher::ACTION_LIST,array());
            }
        }
        //------------------------------------
    ...
    private function immediatelyChangedProducts()
    {
        ...
        foreach ($changedListingsProductsVariationsOptions as $changedListingProductVariationOption) {
            ...
            if ($listingProductVariationOption->getListingProduct()->isRelistable()) {
                ...
                $tempParams = array();
                if ($listingProductVariationOption->getListingProduct()->getSynchronizationTemplate()->getChildObject()->isRelistSendData()) {
                    $tempParams = array('all_data'=>true);
                } else {
                    $tempParams = array('only_data'=>array('base'=>true));
                }

                $this->_runnerActions->setProduct($listingProductVariationOption->getListingProduct(),Ess_M2ePro_Model_Connector_Server_Ebay_Item_Dispatcher::ACTION_RELIST,$tempParams);
            } else if ($listingProductVariationOption->getListingProduct()->isListable()) {
                $this->_runnerActions->setProduct($listingProductVariationOption->getListingProduct(),Ess_M2ePro_Model_Connector_Server_Ebay_Item_Dispatcher::ACTION_LIST,array());
            }
        }
        //------------------------------------
    ...
    private function scheduledListings(&$listings)
    {
        ...
        $listingsProductsCollection = Mage::helper('M2ePro/Component_Ebay')->getModel('Listing_Product')->getCollection();
        ...
        foreach ($listingsProductsArray['items'] as $listingProductArray) {
            ...
            /** @var $listingProduct Ess_M2ePro_Model_Listing_Product */
            $listingProduct = Mage::helper('M2ePro/Component_Ebay')->getObject('Listing_Product',$listingProductArray['id']);
    ...
    private function isMeetRelistRequirements(Ess_M2ePro_Model_Listing_Product $listingProduct)
    {
        ...
        if ($listingProduct->isRelistable() && $this->_runnerActions->isExistProductAction($listingProduct,Ess_M2ePro_Model_Connector_Server_Ebay_Item_Dispatcher::ACTION_RELIST,array())) {
            return false;
        } else if ($listingProduct->isListable() && $this->_runnerActions->isExistProductAction($listingProduct,Ess_M2ePro_Model_Connector_Server_Ebay_Item_Dispatcher::ACTION_LIST,array())) {
            return false;
        }
        //--------------------
