...
class Ess_M2ePro_Model_Ebay_Synchronization_Tasks_Defaults_UpdateListingsProducts extends Ess_M2ePro_Model_Ebay_Synchronization_Tasks
{
    ...
    {
        return Mage::helper('M2ePro/Module')->getConfig()->getGroupValue('/ebay/synchronization/settings/defaults/update_listings_products/','since_time');
    }
    ...
    private function setEbayCheckSinceTime($time)
    {
        ...
        Mage::helper('M2ePro/Module')->getConfig()->setGroupValue('/ebay/synchronization/settings/defaults/update_listings_products/','since_time',$time);
    }
    ...
    private function getChangedListingsProductsForAccount($account, $sinceTime)
    {
        ...
        foreach ($changedItems as $changeItem) {
            ...
            if ($tempListingProductModel->getParentObject()->getStatus() != Ess_M2ePro_Model_Listing_Product::STATUS_LISTED) {
                continue;
            }
            //--------------------------
            ...
            //--------------------------
            $changedListingsProducts[] = $this->prepareChangedListingsProducts($tempListingProductModel->getParentObject(),$changeItem);
            //--------------------------
        }
        //---------------------------
    ...
    private function prepareChangedListingsProducts(Ess_M2ePro_Model_Listing_Product $tempListingProductModel, $ebayChange)
    {
        ...
        if ($tempEbayListingProductModel->isListingTypeAuction()) {
            $tempEbayChanges['online_start_price'] = (float)$ebayChange['currentPrice'] < 0 ? 0 : (float)$ebayChange['currentPrice'];
        }
        if ($tempEbayListingProductModel->isListingTypeFixed()) {
            $tempEbayChanges['online_buyitnow_price'] = (float)$ebayChange['currentPrice'] < 0 ? 0 : (float)$ebayChange['currentPrice'];
        }
        ...
        if (($ebayChange['listingStatus'] == self::EBAY_STATUS_COMPLETED || $ebayChange['listingStatus'] == self::EBAY_STATUS_ENDED) &&
            $tempEbayChanges['online_qty'] == $tempEbayChanges['online_qty_sold']) {
        ...
        foreach ($ebayChange['variations'] as $ebayVariation) {
            ...
            foreach ($tempVariationsWithOptions as $M2eProVariation) {
                ...
                if ($equalVariation === true && count($M2eProVariation['options']) == count($ebayVariation['specifics'])) {
                    ...
                    $tempEbayChanges['online_price'] = (float)$ebayVariation['price'] < 0 ? 0 : (float)$ebayVariation['price'];
