...
class Ess_M2ePro_Model_Amazon_Synchronization_Tasks_Templates_Relist extends Ess_M2ePro_Model_Amazon_Synchronization_Tasks
{
    ...
    private function immediatelyChangeAmazonStatus()
    {
        ...
        foreach ($changedListingsProducts as $changedListingProduct) {
            ...
            if ($listingProduct->isStopped()) {
                $this->_runnerActions->setProduct($listingProduct,Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_RELIST,array());
            } else if ($listingProduct->isNotListed()) {
                $this->_runnerActions->setProduct($listingProduct,Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_LIST,array());
            }
        }
        //------------------------------------
    ...
    private function immediatelyChangedProducts()
    {
        ...
        foreach ($changedListingsProductsVariationsOptions as $changedListingProductVariationOption) {
            ...
            if (!$inspector->isMeetRelistRequirements($listingProductVariationOption->getListingProduct())) {
                continue;
            }

            if ($listingProductVariationOption->getListingProduct()->isStopped()) {
                $this->_runnerActions->setProduct($listingProductVariationOption->getListingProduct(),Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_RELIST,array());
            } else if ($listingProductVariationOption->getListingProduct()->isNotListed()) {
                $this->_runnerActions->setProduct($listingProductVariationOption->getListingProduct(),Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_LIST,array());
            }
        }
        //------------------------------------
    ...
    private function scheduledListings(&$listings)
    {
        ...
        $listingsProductsCollection = Mage::helper('M2ePro/Component_Amazon')->getModel('Listing_Product')->getCollection();
        ...
        foreach ($listingsProductsArray['items'] as $listingProductArray) {
            ...
            /** @var $listingProduct Ess_M2ePro_Model_Listing_Product */
            $listingProduct = Mage::helper('M2ePro/Component_Amazon')->getObject('Listing_Product',$listingProductArray['id']);
