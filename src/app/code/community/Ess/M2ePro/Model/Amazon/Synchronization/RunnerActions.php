...
class Ess_M2ePro_Model_Amazon_Synchronization_RunnerActions
{
    private $_actionsProducts = array();

    // ########################################

    public function setProduct(Ess_M2ePro_Model_Listing_Product $listingProductInstance, $action, array $params = array())
    {
        ...
        if (!is_null($tempExistItem)) {
            ...
            do {
                ...
                if ($action == Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_STOP) {
                    $this->deleteProduct($tempExistItem['instance']);
                    break;
                }

                if ($tempExistItem['action'] == Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_STOP) {
                    return false;
                }

                if ($action == Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_LIST) {
                    $this->deleteProduct($tempExistItem['instance']);
                    break;
                }

                if ($tempExistItem['action'] == Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_LIST) {
                    return false;
                }

                if ($action == Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_RELIST) {
                    $this->deleteProduct($tempExistItem['instance']);
                    break;
                }

                if ($tempExistItem['action'] == Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_RELIST) {
                    return false;
                }
                            ...
                            $percentsFrom = 0, $percentsTo = 100)
    {
        ...
        if ($totalCount == 0) {
            return Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Requester::STATUS_SUCCESS;
        }
        //----------------------------
        ...
        foreach ($actions as $action=>$combinations) {
            foreach ($combinations as $combination) {
                ...
                for ($i=0; $i<count($combination['items']);$i+=$maxCountPerStep) {
                    ...
                    //-----------------------------
                    if (!is_null($lockItem)) {
                        $statusActionTitle = Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::getActionTitle($action);
                    ...
                    $tempResult = Mage::getModel('M2ePro/Connector_Xfabric_Amazon_Product_Dispatcher')->process($action, $itemsForStep, $combination['params']);
        ...
        return Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Requester::getMainStatus($results);
    }
    ...
    {
        $actions = array(
            Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_LIST => array(),
            Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_RELIST => array(),
            Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_REVISE => array(),
            Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Dispatcher::ACTION_STOP => array()
        );
