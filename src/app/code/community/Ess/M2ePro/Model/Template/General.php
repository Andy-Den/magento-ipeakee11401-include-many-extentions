...
class Ess_M2ePro_Model_Template_General extends Ess_M2ePro_Model_Component_Parent_Abstract
{
    ...
    public function getAccount()
    {
        if (is_null($this->accountModel)) {
            $this->accountModel = Mage::helper('M2ePro/Component')->getComponentObject($this->getComponentMode(),'Account',$this->getAccountId());
        }
    ...
    public function getMarketplace()
    {
        if (is_null($this->marketplaceModel)) {
            $this->marketplaceModel = Mage::helper('M2ePro/Component')->getComponentObject($this->getComponentMode(),'Marketplace',$this->getMarketplaceId());
        }
    ...
    {
        return $this->getRelatedComponentItems('Listing','template_general_id',$asObjects,$filters);
    }
