...
abstract class Ess_M2ePro_Model_Connector_Xfabric_Amazon_Product_Requester extends Ess_M2ePro_Model_Connector_Xfabric_Amazon_Requester
{
    ...
    protected function getResponserParams()
    {
        ...
        foreach ($this->listingsProducts as $listingProduct) {
            ...
            $tempXfabricData = array();
            isset($this->listingProductRequestsData[$listingProduct->getId()]['xfabric_data']) &&
                $tempXfabricData = $this->listingProductRequestsData[$listingProduct->getId()]['xfabric_data'];

            $tempProductsData[$listingProduct->getId()] = array(
                'id' => $listingProduct->getId(),
                'request' => array(
                    'native_data' => $tempNativeData,
                    'xfabric_data' => $tempXfabricData
                )
            );
        }
    ...
    protected function setLocks($messageHash)
    {
        ...
        foreach ($this->listingsProducts as $listingProduct) {
            ...
            $listingProduct->addObjectLock(NULL,$messageHash);
            $listingProduct->addObjectLock('in_action',$messageHash);
            $listingProduct->addObjectLock($actionIdentifier.'_action',$messageHash);

            if (isset($tempListings[$listingProduct->getListingId()])) {
                continue;
            }

            $listingProduct->getListing()->addObjectLock(NULL,$messageHash);
            $listingProduct->getListing()->addObjectLock('products_in_action',$messageHash);
            $listingProduct->getListing()->addObjectLock('products_'.$actionIdentifier.'_action',$messageHash);

            $tempListings[$listingProduct->getListingId()] = true;
        }

        $this->account->addObjectLock('products_in_action',$messageHash);
        $this->account->addObjectLock('products_'.$actionIdentifier.'_action',$messageHash);

        $this->marketplace->addObjectLock('products_in_action',$messageHash);
        $this->marketplace->addObjectLock('products_'.$actionIdentifier.'_action',$messageHash);
    }
