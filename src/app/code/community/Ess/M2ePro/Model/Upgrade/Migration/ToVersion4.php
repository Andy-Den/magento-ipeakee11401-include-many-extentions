...
SQL
        ...
        $this->installer->getConnection()->update($newGeneralTable, array('object_type' => 3), array('`object_type` = ?'=>1+5));
        $this->installer->getConnection()->update($newGeneralTable, array('object_type' => 4), array('`object_type` = ?'=>2+5));
        $this->installer->getConnection()->update($newGeneralTable, array('object_type' => 2), array('`object_type` = ?'=>3+5));
    }
    ...
    private function convertMagentoOrdersSettings(array $singleAccountSettings)
    {
        $newFormat = array(
            ...
            'status_mapping' => array(
                ...
                'new' => !empty($singleAccountSettings['orders_status_checkout_completed']) ? $singleAccountSettings['orders_status_checkout_completed'] : 'pending',
                'paid' => !empty($singleAccountSettings['orders_status_payment_completed']) ? $singleAccountSettings['orders_status_payment_completed'] : 'processing',
                'shipped' => !empty($singleAccountSettings['orders_status_shipping_completed']) ? $singleAccountSettings['orders_status_shipping_completed'] : 'complete',
            ),
