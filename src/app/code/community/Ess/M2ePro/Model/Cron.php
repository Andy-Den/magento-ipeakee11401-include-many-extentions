...
class Ess_M2ePro_Model_Cron
{
    ...
    public function isNotRunVeryLong()
    {
        if (!Mage::getSingleton('M2ePro/Wizard')->isInstallationFinished()) {
            return false;
        }
    ...
    private function processXfabric()
    {
        if (!$this->isNowTimeToRun(self::TASK_XFABRIC)) {
            return;
        }

        try {
            Mage::getModel('M2ePro/Xfabric_Cron')->process();
