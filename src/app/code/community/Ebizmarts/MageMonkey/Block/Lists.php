...
class Ebizmarts_MageMonkey_Block_Lists extends Mage_Core_Block_Template
{
	...
	public function listLabel($list)
	{
        ...
        if(Mage::getSingleton('core/design_package')->getPackageName() == 'ultimo') {
            $result = $checkbox->getElementHtml() . $checkbox->getLabelHtml();
        } else {
            $result = $checkbox->getLabelHtml() . $checkbox->getElementHtml();
        }
		return $result;
	}
}
