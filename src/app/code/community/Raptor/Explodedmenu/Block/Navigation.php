...
{
	public function drawItem($category, $ij='', $level=0, $last=false) {
		$html = '';
		if (!$category->getIsActive()) {
			return $html;
		}
		$activeChildren = $this->getActiveChildren($category);
		$html.= "<li class='exmenu".$ij." level0'";
		...
		static $count = 0;
		if (sizeof($activeChildren) > 0) {
			$html .= $this->drawColumns($activeChildren,$count,$ij);
		}
		$html .= "</li>";
		$count++;
		return $html;
	 ...
	 * @return unknown
	 */
	public function drawColumns($children,$menunum,$ij) {
		try  {
		...
		$html .= "<ul class='subexmenu".$ij." submenu'>";
		$i = 0;
		...
		$html .='<li class="aside">
                    ...
                    <b>Use our vacfinder <br>or <a href="/store-locator">find a store</a></b></p>
                    <ul>
                 		<li class="i1">
                        	<a href="#" class="product-image" id="product-image-'.$menunum.'"><img name="category_image_'.$menunum.'" id="category_image_'.$menunum.'" src="/media/spacer.gif" alt="" style="display:none;"></a>
			...
			if ($child->getIsActive()) {
				if($level == 2){
					...
					if(sizeof($this->getActiveChildren($child))>0){
						$html .= '<dt><a onmouseover="changeImage(\''.$this->getImageUrl($child).'\',\''.$child->getName().'\',\''.$menunum.'\');" href="'.$this->getCategoryUrl($child).'"><span>'.$this->htmlEscape($child->getName()).'</span></a></dt>';
					}
					else{
						$html .= '<dt><a onmouseover="changeImage(\''.$this->getImageUrl($child).'\',\''.$child->getName().'\',\''.$menunum.'\');" href="'.$this->getCategoryUrl($child).'"><span>'.$this->htmlEscape($child->getName()).'</span></a></dt>';
					}
