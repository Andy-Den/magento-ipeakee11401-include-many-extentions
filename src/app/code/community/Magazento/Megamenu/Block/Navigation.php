...
class Magazento_Megamenu_Block_Navigation extends Mage_Core_Block_Template {
	...
	const CACHE_TAG             = 'magazento_megamenu';
    ...
    protected function _construct() {
    	...
    	$currency = $_store->getCurrentCurrencyCode();
    	$secure = $_store->isCurrentlySecure() ? 'secure': 'unsecure';
    	$storeCode = $_store->getCode();
    	
        $this->addData(array(
            'cache_lifetime' => 86400,
            'cache_tags' => array(Mage_Core_Model_Store::CACHE_TAG, self::CACHE_TAG),
            'cache_key' => $currency.'-'.$secure . '-'. $storeCode,
        ));
    ...
    private function getCategoryUrl($categoryId) {
    	...
    	if (isset($this->_categories[$categoryId])) {
    		$obj = $this->_categories[$categoryId];
    		return $obj->getUrl();
    	}
    	return '';
    }
    
    private function getCategoryParentIdByCategoryId($categoryId) {
    	if (isset($this->_categories[$categoryId])) {
    		$obj = $this->_categories[$categoryId];
    ...
    private function getProductCount($categoryId) {
    	if (isset($this->_categories[$categoryId])) {
    		$obj = $this->_categories[$categoryId];
    ...
    public function drawCatalog() {
        ...
        foreach ($data as $key => $item) {
            ...
            if (!isset($this->_categories[$categoryId])) continue;

            $categoryParentId = $this->getCategoryParentIdByCategoryId($categoryId);
    ...
    public function drawNestedMenus($children, $level=1,$morehref ='') {
        ...
        foreach ($children as $child) {
            if ($child->getIsActive()) {
                ...
                $url = $this->getCategoryUrl($child->getId());
    ...
    public function drawAllCategoriesMenu() {
        ...
        foreach ($collection as $node) {
            if ($node->getData('level') == 2) {
                $html .= '<li>';
                $url = $this->getCategoryUrl($node->getId());
