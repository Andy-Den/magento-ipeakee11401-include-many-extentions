...
class Celebros_Salesperson_Helper_Data extends Mage_CatalogSearch_Helper_Data
{
	...
	public function getQueryText()
	{
		if (is_null($this->_queryText)) {
			$this->_queryText = $this->getUrlParam($this->getQueryParamName());
	...
	{
        return $this->sanitizeOutput($this->getQueryText()); 
	}
	...
	public function getResultUrl($query = null, $bAddIndexSuffix = false, $bRelativePath = false)
	{
		...
		$helperPath =  $module_disabled ? 'catalogsearch/result' : $this->getFrontname()    .'/result';
	...
	public function updateCatUrlRewrites($store_id) // For Magento < 1.13
	{
		$pattern = 'category/';
		
		$categories = Mage::getModel('catalog/category')->getCollection()
       	->addNameToResult()
		->addAttributeToSelect(array('salesperson_search_phrase'))
		...
		{
            if ($categoryLookup[(int) ($rewrite->getData('category_id'))]->getData('salesperson_use_in_nav'))
            {
			$rewrite->delete();
	...
	public function updateCatUrlRewritesNewMethod($store_id=null, $enable) // For Magento >= 1.13
	{
        Mage::app()->setCurrentStore($store_id);
		// Get categories for current store
		$categpryHelper = Mage::helper('catalog/category');
		$categoriesCollection = $categpryHelper->getStoreCategories('name', true, false);
		$categoriesCollection->addAttributeToSelect(array('salesperson_search_phrase'));
		$categoriesCollection->addAttributeToSelect(array('salesperson_use_in_nav'));
		$urlRewrite=Mage::getModel('enterprise_urlrewrite/url_rewrite');
		$func= $enable ? "getCategoryRewriteQuery" : "getMagentoCategoryRewriteQuery";
		foreach ($categoriesCollection as $category) {
		    ...
		    if ($isCatInNav)
		    {
		        ...
		        if (isset($requestPath)) {
	                $urlRewrite->loadByRequestPath(array($requestPath));
	                $query=$this->$func($category);
		            if ($enable) {
		            	$targetPath = Mage::helper('salesperson')->getResultUrl($query, $bAddIndexSuffix = true, $bRelativePath = true);
		            }
		            else {
	...
	public function updateCategoriesUrlRewrites($store_id=null/*, $enable=true*/) {
		//$mageVersionInfo=Mage::getVersionInfo();
       	//$isNewMethod=( ($mageVersionInfo['major']==1) && ($mageVersionInfo['minor']>=13) ) ? true : false;
       	//if ($isNewMethod)
       	//	$this->updateCatUrlRewritesNewMethod($store_id, $enable);
       	//else
       		$this->updateCatUrlRewrites($store_id);
	}
	...
	public function getMagentoCategoryRewriteQuery($category)
	{
	...
	{
		return Mage::getStoreConfigFlag('salesperson/display_settings/campaigns_enabled');
	}
    public function getFrontname()
    {
        return (string) Mage::getConfig()->getNode('frontend/routers/salesperson/args/frontName');
    }
