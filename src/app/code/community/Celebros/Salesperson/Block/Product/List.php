...
class Celebros_Salesperson_Block_Product_List extends Celebros_Salesperson_Block_Product_Abstract
{
    ...
    protected function _getProductCollection()
    {
        if (is_null($this->_productCollection)) {
            $_productCollection = $this->getQwiserSearchResults()->Products->Items;
            ...
            if (sizeof($prodIds)) {
                $mageCollection = Mage::getResourceModel('catalog/product_collection')->addFieldToFilter('entity_id', $prodIds);
                $mageCollection = $this->_addProductAttributesAndPrices($mageCollection);

                foreach ($_productCollection as $item) {
                    foreach ($mageCollection as $key=>$mageItem) {
                        if ($item->Field['id'] == $mageItem->getEntityId()) {
                            $item->Mage_product = $mageItem;
                        }
                    }
                }
            }
