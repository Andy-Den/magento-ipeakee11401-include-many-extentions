...
class Celebros_Salesperson_Block_Layer_View extends Mage_Core_Block_Template
{
    ...
    public function getFilters()
    {
    	...
    	if(!isset($filters)){
			...
			$this->saveToCacheFiltersText($filters);
    	}
        return $filters;
    }

	protected function saveToCacheFiltersText($filters){
    	/*$cache = Mage::app()->getCache();
    	$filtersText = unserialize($cache->load("salesperson_filters_text_cache"));
    	var_dump($cache->load("salesperson_filters_text_cache"));
    	if(!isset($filtersText)) $filtersText = array();
    	foreach($filters as $filter) {
    		$filtersText[$filter->Id] = $filter->SideText;
    	}
    	$cache->save(serialize($filtersText), "salesperson_filters_text_cache", array("salesperson_cache"), 60*60*60);*/
    	
    	$filtersText = isset($_COOKIE["salesperson_filters_text_cache"]) ? unserialize($_COOKIE["salesperson_filters_text_cache"]) : array();
    	foreach($filters as $filter) {
    		$filtersText[$filter->Id] = $filter->SideText;
    	}
    	setcookie('salesperson_filters_text_cache', serialize($filtersText));
    }
    ...
    public function answerQuestionsUrl(){
        ...
        $frontname=Mage::getConfig('frontend/routers/salesperson/args/frontName');

    	$url =  Mage::getUrl($frontname.'/result/change', $urlParams);
    	if (preg_match("/p==*\d/", $url)){
    		$url = preg_replace("/p==*\d/",'p=1', $url);
    	}
    	else {
...
//	    	$html .=
//	    	"<input type=te {$strChecked} {$strDisabled}
