...
class Celebros_Salesperson_Model_Observer
{
    ...
    protected $_log_file_name = 'celebros.log';
    protected $_log_file_max_size = 2; // in MB
    ...
    protected $_isReviewEnabled; // Is the Magento review module enabled
    ...
    protected function logProfiler($msg, $isSpaceLine = false)
    {
        ...
        $log_dir = Mage::getBaseDir('var') . DS . 'log';
        $log_file = $log_dir . DS . $this->_log_file_name;
        if (file_exists($log_file)) {
            $log_size = filesize($log_file) / 1024 / 1024;
            if ($log_size > $this->_log_file_max_size) {
                $fp = fopen($log_file, 'a');
                ftruncate($fp, 0);
                fclose($fp);
            }
        }
        Mage::log($msg, null, $this->_log_file_name, true);

        if ($isSpaceLine)
            Mage::log('', null, $this->_log_file_name, true);
    }
     ...
     */
    public function catalogUpdate($observer = null)
    {
        ...
        if ($enabled) $this->export_celebros(false);
    ...
    public function export_celebros($webAdmin)
    {
        ...
        foreach ($store_view_ids as $store_view_id) {
            ...
            $this->_fileNameZip = Mage::getStoreConfig('salesperson/export_settings/zipname', $store_view_id["store_id"]);

            $this->logProfiler("Store code: {$this->_fStore->getCode()}, name: {$this->_fStore->getName()}");
            $this->logProfiler("Zip file name: {$this->_fileNameZip}");

            $this->comments_style('section', "Store code: {$this->_fStore->getCode()}, name: {$this->_fStore->getName()}", 'STORE');
            ...
            if (!is_null($total_min)) {
                ...
                while ($count > 0) {
                    ...
                    //$this->comments_style('info','Info: max:'. $max,'info');
    ...
    public function content($product, $attributes, $cate, $typ, $chil, $config, $children, $nameCategory, $childrenName)
    {
        ...
        foreach ($attributes as $att_code => $header_metadata) {
            ...
            if (($att_code == 'thumbnail') && (stristr($products[$header_index], 'notice') != false)) {
                $this->comments_style('warning', 'THUMBNAIL ERROR: Product ID ' . $product->getId() . ', ' . $products[$header_index], 'warning');
                $this->logProfiler('THUMBNAIL ERROR: Product ID ' . $product->getId() . ', ' . $products[$header_index]);
        ...
        switch ($att_type) {
            ...
            case "media_image":
                try {
                    ...
                    $attributeValue = 'NOTICE: No image is set for product and image placeholder is not set';
                }
                break;
            ...
            default:
                ...
                elseif ($att_code === "thumbnail") {
                    try {
                        $attributeValue = Mage::helper('catalog/image')->init($product, 'thumbnail')->resize(66);
    ...
    public function export_config($store_id)
    {
        ...
        $this->_isReviewEnabled = Mage::getConfig()->getModuleConfig('review/review')->is('active', 'true');
        //feature is not in use
    ...
    protected function _copyImageToSalespersonCatalog($imageUrl, $productId)
    {
        ...
        $catalogRelativeImagePath = substr($imageUrl, $pos);
        ...
        if (file_exists($sourceImagePath)) {
            ...
            if (!is_dir($destinationFolder)) {
                if (!mkdir($destinationFolder, 0777, true)) {
                    $this->comments_style('error', 'Error: Failed to create directory' . $destinationFolder . ' ...', 'error');
                    exit();
                }
            }
            ...
            if (!copy($sourceImagePath, $destinationImagePath)) {
                $this->comments_style('error', 'Error: Failed to copy ' . $sourceImagePath . 'to' . $destinationImagePath . '...', 'error');
                exit();
            }
        } else {
            $this->comments_style('warning', 'IMAGE ERROR: Product ID ' . $productId . ', image url: ' . $imageUrl, 'warning');
            $this->logProfiler('IMAGE ERROR: Product ID ' . $productId . ', image url: ' . $imageUrl);
            return '';
        }

        $resultUrl = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . "sales_catalog" . DIRECTORY_SEPARATOR . "product" . DIRECTORY_SEPARATOR . $productId . DIRECTORY_SEPARATOR . $fileName;
        return $resultUrl;
    }
    ...
    public function export_orders_celebros()
    {
        ...
        foreach ($store_view_ids as $store_view_id) {
            ...
            $bCrossSellEnabled = (Mage::getStoreConfigFlag('salesperson/crosssell_settings/crosssell_enabled') || Mage::getStoreConfigFlag('salesperson/crosssell_settings/upsell_enabled'));

            if (!$bCrossSellEnabled)
                continue;
            ...
            $zipFileName = Mage::getStoreConfig('salesperson/export_settings/datahistoryname', $store_view_id["store_id"]);
    ...
    public function url_rewrite_reindex()
    {
        ...
        foreach ($store_view_ids as $store_view) {
            ...
            $this->logProfiler("Checking need for reindexing of URL rewrite for store: {$store_view_name}, ID:{$store_view_id}");
            ...
            if (Mage::getStoreConfigFlag('salesperson/nav_to_search_settings/nav_to_search', $store_view_id)) {
                ...
                Mage::helper('salesperson')->updateCategoriesUrlRewrites($store_view_id, true);
                $rewrite_end = (float)array_sum(explode(' ', microtime()));

                $this->logProfiler("Reindexed URL rewrite for store: {$store_view_name}, ID:{$store_view_id}. Total Time: " . round($rewrite_end - $rewrite_start, 1) . ' sec.');
            }
        }
    }
    ...
    public function after_reindex_process_catalog_url($observer)
    {
        ...
        if (!$isNewMethod) {
            $this->logProfiler("Event observer: after_reindex_process_catalog_url");
            $this->url_rewrite_reindex();
        } else
            return; // Do nothing
    }

    public function catalog_controller_category_init_after(Varien_Event_Observer $observer)
    {
        $mageVersionInfo = Mage::getVersionInfo();
        ...
        $isNavEnabled = Mage::getStoreConfigFlag('salesperson/nav_to_search_settings/nav_to_search', $store_id);

        if ($isNavEnabled) {
            $category = Mage::registry('current_category');

            $isCatInNav = (bool)$category->getData('salesperson_use_in_nav');

            if ($isCatInNav) {
                $helper = Mage::helper('salesperson');

                $query = $helper->getCategoryRewriteQuery($category);

                $request = Mage::app()->getFrontController()->getRequest();

                $request->setModuleName('salesperson');
                $request->setControllerName('result');
                $request->setActionName('index');
                $request->setParams(array('q' => $query));
    ...
    public function showMessageAddCart()
    {
    ...
    public function showMessageAddCompare()
    {
        $product = Mage::getModel('catalog/product')->load(Mage::app()->getRequest()->getParam('product', 0));
        ...
        Mage::getSingleton('core/session')->setProductToCompare(
            array(
                'name' => $product->getName())
             );
    }
