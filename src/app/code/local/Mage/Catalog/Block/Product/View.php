...
class Mage_Catalog_Block_Product_View extends Mage_Catalog_Block_Product_Abstract
{
    ...
    public function getProduct()
    {
        ...
        if( Mage::registry('product')) {
            $current_product_id = Mage::registry('product')->getId();
            //Start code To get the Warrantied products
            $read = Mage::getSingleton('core/resource')->getConnection('core_read');
            $value = $read->query("Select * from catalog_product_link WHERE product_id='" . $current_product_id . "' AND link_type_id='7'");
            $products = array();
            while ($row = $value->fetch()) {
                $obj = Mage::getModel('catalog/product');
                $cus_product = $obj->load($row['linked_product_id']);
                $products[] = $cus_product;
            };
            //End code to get the warrantied products
            Mage::registry('product')->_data['warrantied_products'] = $products;
        }
