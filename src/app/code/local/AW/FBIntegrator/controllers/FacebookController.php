...
class AW_FBIntegrator_FacebookController extends Mage_Core_Controller_Front_Action {
    ...
    public function connectAction() {
        ...
        if ($me && isset($me['email'])) {
                ...
                if ($customer->getId()) {
                    ...
                    try {
                        ...
                        if ($customer->getId()) {
                            ...
                            if($this->onestepcheckoutEnable() && $session->getBeforeAuthUrl() == Mage::getUrl("onestepcheckout"))
                            {
                                Mage::getSingleton('core/session')->addError($helper->__('Thank you for registering with %s.', Mage::app()->getStore()->getFrontendName()));
                            }
                        }
                    } catch (Exception $exc) {
                        ...
                        if($this->onestepcheckoutEnable() && $session->getBeforeAuthUrl() == Mage::getUrl("onestepcheckout"))
                        {
                            Mage::getSingleton('core/session')->addError($helper->__('Error during registering with %s.', Mage::app()->getStore()->getFrontendName()));
                        }
                    }
                }
            }
            ...
            if($this->onestepcheckoutEnable() && $session->getBeforeAuthUrl() == Mage::getUrl("onestepcheckout"))
            {
                Mage::getSingleton('core/session')->addError($helper->__('Unable to log in with Facebook. Please, try again later.'));
            }
        }
    ...
    protected function _loginPostRedirect() {
        $session = Mage::getSingleton('customer/session');
        if(!$this->onestepcheckoutEnable() &&  $session->getBeforeAuthUrl())
        {
    ...
    public function onestepcheckoutEnable()
    {
        return (Mage::getConfig()->getNode()->modules->Idev_OneStepCheckout && Mage::getStoreConfig('onestepcheckout/general/rewrite_checkout_links'));
    }
