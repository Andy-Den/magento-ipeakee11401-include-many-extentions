...
//            Mage::app()->getStore()->setConfig('design/theme/'.$type, $this->_mobileVal);
    ...
    public function predispatch($event) {
        ...
        /* Compatibility with Magento EE full page cache */
        if (Mage::getConfig()->getNode('modules/Enterprise_Enterprise')) {
            ...
            $basePath = $httpRequest->getBasePath();
            $params = $httpRequest->getParams();

            if (isset($params['___store'])) {
                $storeCode = $params['___store'];
                Mage::getModel('core/cookie')->set(self::COOKIE_NAME, $storeCode, true, '/');
            } else if ($cookie = $this->getCookie()->get(self::COOKIE_NAME)) {
                $storeCode = $cookie;
            } else {
                $storeCode = Mage::app()->getStore()->getId();
            }

            $rewriteCookie = "{$packageName}_{$themeName}_{$storeCode}";

            Mage::app()->setCurrentStore($storeCode);

            if ($httpRequest->getControllerName() == 'switch' && ($httpRequest->getActionName() == 'toMobile' || $httpRequest->getActionName() == 'toDesktop')) {
                $this->getCookie()->delete(Mage_Core_Model_Store::COOKIE_NAME);
                $this->getCookie()->set(Mage_Core_Model_Store::COOKIE_NAME, $rewriteCookie, true, '/');
            } 
        }

        if (Mage::getDesign()->getArea() == 'adminhtml' || 'api' == $httpRequest->getModuleName()) {
            return;
        }
        if ($this->_helper()->getTargetPlatform() == self::TARGET_MOBILE) {
