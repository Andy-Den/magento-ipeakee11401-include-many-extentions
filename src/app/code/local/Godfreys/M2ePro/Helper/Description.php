<?php
/**
 * Generate ebay description
 *
 * Custom description.
 *
 * @category  Godfreys
 * @package   Godfreys_M2ePro
 * @author    Balance Internet Team <dev@balanceinternet.com.au>
 * @copyright 2015 Balance
 */

class Godfreys_M2ePro_Helper_Description extends Ess_M2ePro_Helper_Module_Renderer_Description
{
    /** Override to add custom description
     *
     * @param string                           $text           description text
     * @param Ess_M2ePro_Model_Magento_Product $magentoProduct magento product
     *
     * @return mixed|string
     */
    public function parseTemplate($text, Ess_M2ePro_Model_Magento_Product $magentoProduct)
    {
        $text = $this->updateDescription($text, $magentoProduct);
     ...
     * @throws Exception
     */
    public function updateDescription( $text, Ess_M2ePro_Model_Magento_Product $magentoProduct )
    {
        ...
        $html='';
        $count = 1;
        //get only frontend attributes of product
        foreach ($attributes as $attributeCode) {
            ...
            if ($value != null && $value !='' && $value !=' ' && $value !='No') {
                if ($count%2==1) {
                    $html  .=  '<tr class="odd">
        ...
        $text = str_replace('#attribute_table#', $html, $text);
        return $text;
    }
    ...
    private function normalizeDescription($str)
    {
        ...
        // Remove all links
        //$str = preg_replace('/<a (href|title|class|target)=(.*?)\>(.*?)<\/a>/', "\\3", $str);

        $allowedTags = array('<p>', '<br>', '<ul>', '<li>', '<b>');

        $str = str_replace(array('<![CDATA[', ']]>'), '', $str);
        $str = strip_tags($str, implode($allowedTags));
