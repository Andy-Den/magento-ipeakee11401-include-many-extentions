...
class SecurePay_Sxml_Model_Sxml extends Mage_Payment_Model_Method_Cc {
    ...
    public function getDebug($storeId = null) {
        return Mage::getStoreConfig('payment/Sxml/debug', $storeId);
    }

    public function isFraudGuard($storeId = null) {
        return Mage::getStoreConfig('payment/Sxml/antifraud', $storeId);
    }

    public function getMode($forceNormal = 0, $storeId = null) {
        $fraud = $forceNormal ? 0 : $this->isFraudGuard($storeId);

        if (Mage::getStoreConfig('payment/Sxml/test', $storeId)) {
    ...
    public function getUsername($storeId = null) {
        return trim(Mage::getStoreConfig('payment/Sxml/username', $storeId));
    }

    public function getPassword($storeId = null) {
        return trim(Mage::getStoreConfig('payment/Sxml/password', $storeId));
    }

    public function getCCVStatus($storeId = null) {
        return Mage::getStoreConfig('payment/Sxml/usecvv', $storeId);
    }
    ...
    public function getStoreId($payment = null) {
        if ($payment) {
            if ($payment->getOrder()) {
                return $payment->getOrder()->getStoreId();
            }
        }
    ...
    public function authorize(Varien_Object $payment, $amount) {
        ...
        $storeId = $this->getStoreId($payment);

        if ($this->getDebug($storeId)) {
            $writer = new Zend_Log_Writer_Stream($this->getLogPath());
            $logger = new Zend_Log($writer);
        }
        ...
        Mage::log($transaction_id . ', ' . $store->getId() . ', ' . $currency_code, Zend_Log::DEBUG, 'debug.log');

        if ($this->isFraudGuard($storeId)) {
            //Create the fraudguard transaction object
            $sxml = new securexml_transaction($this->getMode(0, $storeId), $this->getUsername($storeId), $this->getPassword($storeId));
            ...
            //Issue check
            $approved = $sxml->processCreditFraudCheck($amount, $transaction_id, $payment->getCcNumber(), $payment->getCcExpMonth(), $payment->getCcExpYear(), $payment->getCcCid(), $currency_code);
        }

        if ($approved) {
            //Get the preauth transaction object
            $sxml = new securexml_transaction($this->getMode(NO_ANTIFRAUD, $storeId), $this->getUsername($storeId), $this->getPassword($storeId));
            //Issue the preauth
            $approved = $sxml->processPreauth($amount, $transaction_id, $payment->getCcNumber(), $payment->getCcExpMonth(), $payment->getCcExpYear(), $payment->getCcCid(), $currency_code);
        }
    ...
    public function capture(Varien_Object $payment, $amount) {
        ...
        $order_id = $transaction_id = $payment->getOrder()->getIncrementId();
        Mage::log('order id=' . $order_id, Zend_Log::DEBUG, 'debug.log');

        $store_id = $storeId;//$payment->getOrder()->getStoreId();
        Mage::log('store id=' . $store_id, Zend_Log::DEBUG, 'debug.log');

        $store = Mage::app()->getStore($store_id);
        Mage::log('store id2=' . $store->getId(), Zend_Log::DEBUG, 'debug.log');

        $currency_code = $store->getBaseCurrencyCode();
        Mage::log('currency code= ' . $currency_code, Zend_Log::DEBUG, 'debug.log');
        ...
        if ($txnType == "Advice") {
            ...
            if ($this->isFraudGuard($storeId)) {
                ...
                $approved = $sxml->processCreditFraud($amount, $transaction_id, $payment->getCcNumber(), $payment->getCcExpMonth(), $payment->getCcExpYear(), $payment->getCcCid(), $currency_code);
            } else {
                //Issue a standard transaction
                $approved = $sxml->processCredit($amount, $transaction_id, $payment->getCcNumber(), $payment->getCcExpMonth(), $payment->getCcExpYear(), $payment->getCcCid(), $currency_code);
            }
        }
        ...
        if ($approved) {
            ...
            if ($this->getDebug($storeId)) {
                $logger->info("Advice/Standard Approved. Currency = " . $currency_code . " Order: " . $order_id . " Response ID: " . $payment->getCcTransId());
            }
        } else {
            $error = $sxml->getError();
