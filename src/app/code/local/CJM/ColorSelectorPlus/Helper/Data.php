...
{	
	...
	{
		$swatch_attributes = $this->getSwatchAttributes();
	...
	public function getImageSizes()
	{
		...
		$b = str_replace(" ", "", Mage::getStoreConfig('color_selector_plus/templatesettings/prodimgsize', $store));
        ...
        $m = str_replace(" ", "", Mage::getStoreConfig('color_selector_plus/templatesettings/moreimgsize', $store));
        ...
        $l = str_replace(" ", "", Mage::getStoreConfig('color_selector_plus/templatesettings/listimgsize', $store));
	...
	public function getMoreViews($_product)
	{
		...
		if($product_base):
			...
			if($defaultmoreviews == 1) {
				$html_first = '<h2 id="moreviews-title">'.$this->__('More Views').'</h2><ul id="ul-moreviews" class="thumbnails slides thumbnail-slider">'; }
			else {
				$html_first = '<h2 id="moreviews-title" style="display:none;">'.$this->__('More Views').'</h2><ul id="ul-moreviews" class="thumbnails slides thumbnail-slider">'; }
					...
					foreach($options as $option):
						...
						foreach($images as $key => $image):
							if($product_base['morev'][$key] == $value):
								...
								$product_thumb = $product_base['thumb'][$key];
								$product_label = $product_base['label'][$key];
								$html .= '<li class="moreview'.$value.' cloud-zoom-gallery lightbox-group" style="display:none;"><a href="'.$product_image.'" onclick="$(\'image\').src = this.href; return false;"><img src="'.$product_thumb.'" width="'.$theSizes['more']['width'].'" height="'.$theSizes['more']['height'].'" alt="'.$product_label.'" title="'.$product_label.'" /></a></li>';
							endif;
						endforeach;
               		endforeach;
				endif;
		...
		if($product_base):
			...
			foreach($images as $key => $image):
				if($product_base['defaultimg'][$key] == 1 && $product_base['morev'][$key] != 99999999):
					...
					$product_thumb = $product_base['thumb'][$key];
					$product_label = $product_base['label'][$key];
					$product_gallery = $this->getGalUrl($product_base['id'][$key], $_product->getId());
					$rel = $this->getZoomRel($product_gallery, $product_image, $product_label);
					$onloads .= '<li class="onload-moreviews onload-views"><a href="'.$product_base['full'][$key].'" '.$rel.' class="cloud-zoom-gallery lightbox-group" title="'.$product_label.'"><img src="'.$product_thumb.'" width="'.$theSizes['more']['width'].'" height="'.$theSizes['more']['height'].'" alt="'.$product_label.'" title="'.$product_label.'" /></a></li>';
				elseif($product_base['morev'][$key] == 0):
					$defaultmoreviews = 1;
					$product_image = $product_base['image'][$key];
					$product_thumb = $product_base['thumb'][$key];
					...
					$product_gallery = $this->getGalUrl($product_base['id'][$key], $_product->getId());
					$rel = $this->getZoomRel($product_gallery, $product_image, $product_label);
					$defaults .= '<li class="default-moreviews onload-views"><a href="'.$product_base['full'][$key].'" '.$rel.' class="cloud-zoom-gallery lightbox-group" title="'.$product_label.'"><img src="'.$product_thumb.'" width="'.$theSizes['more']['width'].'" height="'.$theSizes['more']['height'].'" alt="'.$product_label.'" title="'.$product_label.'" /></a></li>';
				endif;
			endforeach;
			...
			if(Mage::getStoreConfig('color_selector_plus/zoom/zoomexists') == true){
                ...
                if ($_product->getData('youtube_url')) {
                    preg_match('/[\\?\\&]v=([^\\?\\&]+)/', $_product->getData("youtube_url"), $matches);
                    $html_first .= "<li><a href='http://www.youtube.com/embed/" . $matches[1] . "' class='youtube'>Video</a></li>";
                }
            }
			$html = $html_first .= $html;
	...
	public function getQueryString($_product)
	{
		...
		if($_product->getTypeId() == 'simple' && isset($parentIdArray[0])) {
			$_productModel = Mage::getModel('catalog/product')->load($_product->getId());
			foreach($swatch_attributes as $attributename) {
				if (isset($this->attributes[$attributename])) {
					$attribute = $this->attributes[$attributename];
					$attributeoption = $attribute->getSource()->getOptionText($_product->getData($attributename));
				}
				else {
					$attributeoption = $_productModel->getAttributeText($attributename);
				}
				if($attributeoption) {
	...
	public function getSwatchAttributes()
	{
		...
		$swatchattributes = Mage::getStoreConfig('color_selector_plus/colorselectorplusgeneral/colorattributes',$store);
	...
	public function getSwatchSize($theCode)
	{
		$store = Mage::app()->getStore();
		if($theCode == 'list'):
			$swatchsize = str_replace(" ", "", Mage::getStoreConfig('color_selector_plus/colorselectorplusgeneral/listsize', $store));
			if (!$swatchsize){
				$swatchsize = '12x12';}
		...
		elseif($theCode == 'shopby'):
			$swatchsize = str_replace(" ", "", Mage::getStoreConfig('color_selector_plus/colorselectorplusgeneral/layersize', $store));
			if (!$swatchsize){
				$swatchsize = '15x15';}
			if(preg_match("/x/i", $swatchsize)){
				return $swatchsize;}
			...
			return $swatchsize;
		elseif($theCode != 'null'):
			$swatchsize = str_replace(" ", "", Mage::getStoreConfig('color_selector_plus/swatchsizes/swatchsize_'.$theCode.'_swatchsizes', $store));
			if($swatchsize){
				if(preg_match("/x/i", $swatchsize)){
					return $swatchsize;}
				else {
				...
				$swatchsize = str_replace(" ", "", Mage::getStoreConfig('color_selector_plus/colorselectorplusgeneral/size',$store));
				if (!$swatchsize){
					$swatchsize = '25x25';}
				if(preg_match("/x/i", $swatchsize)){
					return $swatchsize;}
	...
	public function getSwatchHtml($attributeCode, $atid, $_product)
	{
		...
		$swatchsize = $this->getSwatchSize($attributeCode);
		...
		$_color_swatch = $this->getSortedByPosition($_colors);
		...
		foreach($_color_swatch as $_inner_option_id)
 		{
			...
			if ( count($matches[0]) > 0 )
			{
				...
				if($zoomenabled && $product_image):
					$product_image_full = $this->findColorImage($theId, $product_base, 'color', 'full'); 
					$product_gallery = $this->getGalUrl($this->findColorImage($theId, $product_base, 'color', 'id'), $_product->getId());
					$rel = $this->getZoomRel($product_gallery, $product_image, $frontendlabel);
				...
				if($urloption==$altText)
				{
              	...
              	elseif($this->getSwatchUrl($theId)):
					$swatchimage = $this->getSwatchUrl($theId);
					$html = $html.'<li class="swatchContainer">';
					$html = $zoomStuff ? $html.$zoomStuff : $html;
				...
				elseif($pos !== false):
					...
					$swatchimage = $this->getSwatchUrl('empty');
					$html = $html.'<li class="swatchContainer">';
					$html = $zoomStuff ? $html.$zoomStuff : $html;
