...
class Balance_Ajax_Model_Observer extends Mage_Core_Model_Abstract
{
	...
	public function processAjaxRequest(Varien_Event_Observer $observer)
	{
		...
		//$ajax = Mage::getModel('ajax/ajax'); remove & add fast fix on ee 1.14
        $ajax = new Balance_Ajax_Model_Ajax();
	...
	public function initMessageFlag(Varien_Event_Observer $observer)
	{
		...
		$object->setData('flag', $clear && (Mage::helper('ajax')->isAjax() || Mage::helper('ajax')->shouldSkip() || Mage::app()->getStore()->isAdmin()) );
		return $this;
	}
	
	public function renderAjaxBlock($observer)
	{
		if (Mage::app()->getStore()->isAdmin() || Mage::helper('ajax')->shouldSkip() ) return;
