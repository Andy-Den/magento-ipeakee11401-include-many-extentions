 ...
 */
class Innoexts_Warehouse_Helper_Catalog_Product_Price_Indexer 
    extends Mage_Core_Helper_Abstract 
{
     ...
     * @return string
     */
    public function getBatchPriceIndexTable()
    {
     ...
     * @return string
     */
    public function getBatchSpecialPriceIndexTable()
    {
     ...
     */
    public function addStockJoin($select)
    {
        $tableAlias     = 'cis';
     ...
     * @return Innoexts_Warehouse_Helper_Catalog_Product_Price_Indexer 
     */
    public function addBatchPriceJoin($select, $tableAlias, $table)
    {
        $select->joinLeft(array($tableAlias => $table), implode(' AND ', array(
            "({$tableAlias}.entity_id = e.entity_id)", 
            "({$tableAlias}.stock_id = cis.stock_id)", 
    ...
    public function getBundleTierPriceExpr($write, $priceType, $price)
    {
        ...
        if ($this->getVersionHelper()->isGe1600()) {
            ...
            if ($priceType == Mage_Bundle_Model_Product_Price::PRICE_TYPE_FIXED) {
                $tierPrice      = new Zend_Db_Expr("IF({$tierExpr} IS NOT NULL, ROUND({$price} - ({$price} * ({$tierExpr} / 100)), 2), NULL)");
            } else {
    ...
    public function getBundleFinalPriceExpr($write, $priceType, $price, $specialExpr, $groupPrice = null)
    {
        if ($this->getVersionHelper()->isGe1600()) {
            ...
            if ($priceType == Mage_Bundle_Model_Product_Price::PRICE_TYPE_FIXED) {
                $finalPrice  = new Zend_Db_Expr("IF({$specialExpr} > 0, ROUND($price * ({$specialExpr} / 100), 2), {$price})");
            } else {
                $finalPrice  = new Zend_Db_Expr("0");
            }
        }
    ...
    public function getAggregatedOptionPriceSelect($write, $table)
    {
        ...
        if ($this->getVersionHelper()->isGe1700()) {
            $select->from(array($table), array('entity_id', 'customer_group_id', 'website_id', 
                ...
                'group_price' => 'SUM(group_price)', 'stock_id'));
        } else {
            $select->from(array($table), array('entity_id', 'customer_group_id', 'website_id', 
                'min_price' => 'SUM(min_price)', 'max_price' => 'SUM(max_price)', 'tier_price' => 'SUM(tier_price)', 
                'stock_id'));
    ...
    public function getConfigurableOptionPriceSelect($write, $table)
    {
        ...
        if ($this->getVersionHelper()->isGe1900()) {
            $select->join(array('cw' => $this->getTable('core/website')), 
                '', array())
                ->join(array('csg' => $this->getTable('core/store_group')), 
                    'csg.website_id = cw.website_id AND cw.default_group_id = csg.group_id', array())
                ->join(array('cs' => $this->getTable('core/store')),
    ...
    public function getDownloadableLinkPriceSelect($write, $table)
    {
        ...
        $select = $write->select()
            ...
            ->where('dl.value = ?', 1)
            ->group(array('i.entity_id', 'i.customer_group_id', 'i.website_id', 'i.stock_id'))
            ->columns(array(
                'min_price' => $minPrice,
                'max_price' => $maxPrice, 
                'stock_id' => 'i.stock_id'
