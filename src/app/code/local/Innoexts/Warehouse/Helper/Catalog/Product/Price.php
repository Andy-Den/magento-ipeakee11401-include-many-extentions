 ...
 */
class Innoexts_Warehouse_Helper_Catalog_Product_Price 
    extends Mage_Core_Helper_Abstract 
{
        ...
        if (count($_data)) {
            ...
            foreach ($data as $item) {
                ...
                if (!isset($oldData[$stockId])) {
                    ...
                    $adapter->update($table, $item, array(
                        $adapter->quoteInto('product_id = ?', $productId), 
                        $adapter->quoteInto('stock_id = ?', $stockId), 
                        $adapter->quoteInto('website_id = ?', $websiteId), 
                    ));
    ...
    public function loadCollectionTierPrices($collection)
    {
        if (
            !$collection || 
            !(in_array(get_class($collection), array(
        ...
        if (count($stockIds)) {
            ...
            if (count($batchPrices)) {
                foreach ($stockIds as $stockId) {
                    ...
                    if (
                        !isset($websiteBatchPrices[$stockId]) && 
                        isset($batchPrices[0]) && 
        ...
        if (count($stockIds)) {
            ...
            if (count($batchSpecialPrices)) {
                foreach ($stockIds as $stockId) {
                    ...
                    if (
                        !isset($websiteBatchSpecialPrices[$stockId]) && 
