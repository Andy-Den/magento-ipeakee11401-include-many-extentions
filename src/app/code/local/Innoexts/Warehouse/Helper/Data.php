 ...
 */
class Innoexts_Warehouse_Helper_Data 
    extends Mage_Core_Helper_Abstract 
{
    ...
    /**
     * Address stock priorities
     * 
     * @var array
     */
    protected $_addressStockPriorities = array();
    /**
     ...
     * 
     * @var array of Innoexts_Warehouse_Model_Shippingtablerate_Tablerate_Method
     */
     ...
     * @return float
     */
    public function getProductFinalPrice($product, $stockId)
    {
        $finalPrice = (float) $product->getFinalPrice();
        $stockPrices = $product->getStockPrices();
        if (isset($stockPrices[$stockId])) {
            $stockPrice = $stockPrices[$stockId];
            if ($stockPrice['price_type'] == 'fixed') {
                $finalPrice = ($stockPrice['price'] < $finalPrice) ? round($finalPrice - $stockPrice['price'], 4) : $finalPrice;
            } else {
                $finalPrice = ($stockPrice['price'] < 100) ? round($finalPrice -  ($finalPrice * ($stockPrice['price'] / 100)), 4) : $finalPrice;
            }
        }
        return $finalPrice;
    }
    ...
    {
        return (in_array($this->getFullControllerName(), array(
            'paypal_express', 
    ...
    public function getStockIdByAddress($address)
    {
        ...
        if (!isset($this->_addressStockIds[$hash])) {
            ...
            $select                 = $adapter->select()
                ->from(array('w' => $warehouseTable), array('stock_id'))
                ->joinInner(array('wa' => $warehouseAreaTable), 'w.warehouse_id = wa.warehouse_id')
     ...
     * @return array
     */
    public function getAddressStockPriorities($address)
    {
     ...
     * @return int
     */
    public function getAddressStockPriority($address, $stockId)
    {
        $stockPriority      = null;
     ...
     * @return int
     */
    public function getAddressMinPriorityStockId($address, $stockIds)
    {
        $minStockId         = null;
        ...
        foreach ($stockIds as $stockId) {
            ...
            if (!is_null($stockPriority) && (is_null($minStockPriority) || ($stockPriority < $minStockPriority))) {
                $minStockPriority   = $stockPriority;
                $minStockId         = $stockId;
            }
        }
        return $minStockId;
    ...
    protected function saveChildData(
        $warehouse, $dataTableName, $dataAttributeCode, $dataValueAttributeCode, $dataValueType = 'string'
    )
    {
        $this->getModelHelper()->saveChildData(
            $warehouse, 'Innoexts_Warehouse_Model_Warehouse', 'warehouse_id', 
    ...
    protected function addChildData($warehouse, $array, $dataAttributeCode)
    {
        $this->getModelHelper()->addChildData(
            $warehouse, 'Innoexts_Warehouse_Model_Warehouse', $array, $dataAttributeCode
        );
        return $this;
    }
    /**
    ...
    protected function loadCollectionChildData(
        $collection, $dataTableName, $dataAttributeCode, $dataValueAttributeCode, $dataValueType = 'string'
    )
    {
    ...
    public function saveCustomerGroups($warehouse)
    {
        ...
        return $this->saveChildData(
            $warehouse, 'warehouse/warehouse_customer_group', 'customer_group_ids', 'customer_group_id', 'int'
        );
    }
    /**
    ...
    public function loadCollectionCustomerGroups($collection)
    {
        ...
        return $this->loadCollectionChildData(
            $collection, 'warehouse/warehouse_customer_group', 'customer_group_ids', 'customer_group_id', 'int'
        );
    }
    /**
    ...
    public function saveCurrencies($warehouse)
    {
        ...
        return $this->saveChildData(
            $warehouse, 'warehouse/warehouse_currency', 'currencies', 'currency', 'string'
        );
    }
    /**
     * Add data currencies
    ...
    public function loadCollectionCurrencies($collection)
    {
        ...
        return $this->loadCollectionChildData(
            $collection, 'warehouse/warehouse_currency', 'currencies', 'currency', 'string'
        );
    }
    /**
     * Remove currencies
    ...
    public function saveShippingCarriers($warehouse)
    {
        ...
        return $this->saveChildData(
            $warehouse, 'warehouse/warehouse_shipping_carrier', 'shipping_carriers', 'shipping_carrier', 'string'
        );
    }
    /**
    ...
    public function loadCollectionShippingCarriers($collection)
    {
        ...
        return $this->loadCollectionChildData(
            $collection, 'warehouse/warehouse_shipping_carrier', 'shipping_carriers', 'shipping_carrier', 'string'
        );
    }
    /**
     ...
     * 
     * @return Innoexts_Warehouse_Model_Mysql4_Shippingtablerate_Tablerate_Method_Collection
     */
    public function getShippingTablerateMethodsCollection()
    {
        return Mage::getSingleton('shippingtablerate/tablerate_method')->getCollection();
    }
    /**
     ...
     * 
     * @return array of Innoexts_Warehouse_Model_Shippingtablerate_Tablerate_Method
     */
