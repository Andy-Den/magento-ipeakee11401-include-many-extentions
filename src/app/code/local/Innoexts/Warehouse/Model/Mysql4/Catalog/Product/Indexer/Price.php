 ...
 */
class Innoexts_Warehouse_Model_Mysql4_Catalog_Product_Indexer_Price  
    extends Mage_Catalog_Model_Resource_Eav_Mysql4_Product_Indexer_Price 
{
    ...
    protected function _prepareBatchPriceIndex($entityIds = null)
    {
        ...
        $select = $write->select()
            ...
            ->join(array('cis' => $this->getTable('cataloginventory/stock')), '', array())
        ...
        $select->joinLeft(array('cbgp' => $this->getTable('catalog/product_batch_price')), 
                implode(' AND ', array(
        ...
        if ($helper->getProductPriceHelper()->isWebsiteScope()) {
            $select->joinLeft(
                array('cbp' => $this->getTable('catalog/product_batch_price')), 
                implode(' AND ', array(
    ...
    protected function _prepareBatchSpecialPriceIndex($entityIds = null)
    {
        ...
        $select->joinLeft(array('cbgp' => $this->getTable('catalog/product_batch_special_price')), 
                implode(' AND ', array(
                    '(cbgp.product_id = e.entity_id)', 
                    '(cbgp.stock_id = cis.stock_id)', 
                    '(cbgp.website_id = 0)', 
        ...
        if ($helper->getProductPriceHelper()->isWebsiteScope()) {
            $select->joinLeft(
                array('cbp' => $this->getTable('catalog/product_batch_special_price')), 
                implode(' AND ', array(
                    '(cbp.product_id = e.entity_id)', 
                    '(cbp.stock_id = cis.stock_id)', 
                    '(cbp.website_id = cw.website_id)', 
