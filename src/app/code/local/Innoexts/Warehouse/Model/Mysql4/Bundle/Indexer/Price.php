 ...
 */
class Innoexts_Warehouse_Model_Mysql4_Bundle_Indexer_Price 
    ...
    protected function _calculateBundleSelectionPrice($priceType)
    {
        ...
        if ($this->getVersionHelper()->isGe1600()) {
            ...
            if ($priceType == Mage_Bundle_Model_Product_Price::PRICE_TYPE_FIXED) {
                $priceExpr = new Zend_Db_Expr("IF(IF(bsp.selection_price_type IS NULL, bs.selection_price_type, "
                    ...
                    . "bsp.selection_price_value) / 100), 2), IF(i.special_price > 0, "
                    . "ROUND(IF(bsp.selection_price_value IS NULL, bs.selection_price_value, bsp.selection_price_value) "
                    . "* (i.special_price / 100), 2), IF(bsp.selection_price_value IS NULL, bs.selection_price_value, "
                ...
                $tierExpr = new Zend_Db_Expr("IF(i.base_tier IS NOT NULL, IF(IF(bsp.selection_price_type IS NULL, "
                    ...
                    . "bsp.selection_price_value) / 100)), 2), IF(i.tier_percent > 0, "
                    . "ROUND(IF(bsp.selection_price_value IS NULL, bs.selection_price_value, bsp.selection_price_value) "
                    . "- (IF(bsp.selection_price_value IS NULL, bs.selection_price_value, bsp.selection_price_value) "
                    . "* (i.tier_percent / 100)), 2), IF(bsp.selection_price_value IS NULL, bs.selection_price_value, "
                    . "bsp.selection_price_value))) * bs.selection_qty, NULL)");
            } else {
                $priceExpr = new Zend_Db_Expr("IF(i.special_price > 0, ROUND(idx.min_price * (i.special_price / 100), 2), "
                    . "idx.min_price) * bs.selection_qty");
                $tierExpr = new Zend_Db_Expr("IF(i.base_tier IS NOT NULL, ROUND(idx.min_price * (i.base_tier / 100), 2) "
        ...
        $select = $write->select()
            ...
            ->join(array('idx' => $this->getIdxTable()), '(bs.product_id = idx.entity_id) AND (i.customer_group_id = idx.customer_group_id) AND '.
                '(i.website_id = idx.website_id) AND (i.stock_id = idx.stock_id)', array())
    ...
    protected function _calculateBundleOptionPrice()
    {
        ...
        if ($this->getVersionHelper()->isGe1600()) {
            $select = $write->select()
                ...
                ->group(array('entity_id', 'customer_group_id', 'website_id', 'option_id', 'stock_id', 'is_required', 'group_type'));
            ...
            $select = $write->select()
                ->from(array('i' => $this->_getBundleSelectionTable()),
                    array('entity_id', 'customer_group_id', 'website_id', 'option_id'))
                ->group(array('entity_id', 'customer_group_id', 'website_id', 'option_id', 'stock_id'))
                ->columns(array(
        ...
        if ($this->getVersionHelper()->isGe1600()) {
            ...
            $select = $write->select()
                ...
                ->join(
                    ...
                    . ' AND i.website_id = io.website_id AND (i.stock_id = io.stock_id)',
                    array()
                )
                ->group(array(
                    'io.entity_id', 'io.customer_group_id', 'io.website_id', 
                    'io.stock_id', 'i.tax_class_id', 'i.orig_price', 'i.price'
                ));
            ...
            $select = $write->select()
                ...
                ->join(
                    ...
                    '(i.entity_id = io.entity_id) AND (i.customer_group_id = io.customer_group_id) AND '.
                        '(i.website_id = io.website_id) AND (i.stock_id = io.stock_id)', 
                    array())
                ->group(array('io.entity_id', 'io.customer_group_id', 'io.website_id', 'io.stock_id'))
    ...
    protected function _prepareTierPriceIndex($entityIds = null)
    {
        ...
        $select = $adapter->select()
            ...
            ->join(
                ...
                '(tp.stock_id IS NULL) OR (tp.stock_id = cis.stock_id)', array())
