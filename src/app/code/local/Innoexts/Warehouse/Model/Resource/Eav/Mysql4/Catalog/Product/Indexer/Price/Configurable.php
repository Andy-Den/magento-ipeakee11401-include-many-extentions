 ...
 */
class Innoexts_Warehouse_Model_Resource_Eav_Mysql4_Catalog_Product_Indexer_Price_Configurable 
    extends Mage_Catalog_Model_Resource_Eav_Mysql4_Product_Indexer_Price_Configurable 
{
    ...
    protected function _prepareFinalPriceData($entityIds = null)
    {
        ...
        $select = $write->select()
            ...
            ->joinLeft(array('tp' => $this->_getTierPriceIndexTable()), '(tp.entity_id = e.entity_id) AND '.
           	...
           	->joinLeft(array('gp' => $this->_getGroupPriceIndexTable()),
        ...
        //JING: postion of columns in 'INSERT SELECT' does matter 
        $select->columns(array('group_price' => $groupPrice, 'base_group_price' => $groupPrice ));
    ...
    protected function _applyCustomOption()
    {
        ...
        $select = $write->select()
            ...
            ->joinLeft(array('gp' => $this->_getGroupPriceIndexTable()),
           		'gp.entity_id = i.entity_id AND gp.website_id = cw.website_id AND gp.customer_group_id = i.customer_group_id',
           		array())
    ...
    protected function _applyConfigurableOption()
    {
        ...
        $select = $write->select()
            ...
            ->joinLeft(array('gp' => $this->_getGroupPriceIndexTable()),
           		'gp.entity_id = i.entity_id AND gp.website_id = i.website_id AND gp.customer_group_id = i.customer_group_id',
           		array())
            ->where('le.required_options=0')
            ->group(array('l.parent_id', 'i.customer_group_id', 'i.website_id', 'l.product_id', 'i.stock_id'));
        if ($this->getVersionHelper()->isGe1600()) {
            ...
            $groupPrice     = $write->getCheckSql('gp.price IS NULL', "{$priceExpression}", 'gp.price');
            $select->columns(array(
                'price'      => $priceColumn,
        	...
        	$priceExpression = new Zend_Db_Expr("SUM(IF((@price:=IF(apw.value_id, apw.pricing_value, apd.pricing_value))"
                . " IS NULL, 0, IF(IF(apw.value_id, apw.is_percent, apd.is_percent) = 1, "
                . "ROUND(i.price * (@price / 100), 4), @price)))");
            $select->columns($priceExpression)
            ...
            ->columns('i.stock_id')
            ->columns(new Zend_Db_Expr("IF(gp.price IS NULL, {$priceExpression}, gp.price)"))
            ;
        }
        ...
        $select = $write->select()
            ->from(array($coaTable), array(
                'parent_id', 'customer_group_id', 'website_id', 'MIN(price)', 'MAX(price)', 'MIN(tier_price)', 'stock_id', 'group_price' => 'MIN(group_price)'))
            ->group(array('parent_id', 'customer_group_id', 'website_id', 'stock_id'));
        $query = $select->insertFromSelect($copTable);
        $write->query($query);
        $table  = array('i' => $this->_getDefaultFinalPriceTable());
        $select = $write->select()
